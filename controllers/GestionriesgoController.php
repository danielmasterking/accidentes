...
class GestionriesgoController extends Controller
{
    ...
    {
        $model = GestionRiesgo::find()->orderby('id DESC')->all();
        ...
        foreach ($meses as $key => $value) {
            ...
            ->select(["SUM(dias_incapacidad) Total_dias","COUNT(id) Total_accidente"])
            ->from('gestion_riesgo')
            ->where('month(fecha_accidente)="'.$key.'" ');
            $command = $query->createCommand();
            // Ejecutar el comando:
            $rows = $command->queryOne();

            //$query = GestionRiesgo::find()->where('month(fecha_accidente)="'.$key.'" ')->count();
            $data_mes[]=[$value,(int)$rows[Total_accidente]];
            $data_dias[]=[$value,(int)$rows[Total_dias]];

        }

        $json_mes=json_encode($data_mes);
        $json_dias=json_encode($data_dias);
        /*echo "<pre>";
        print_r($data_mes);
        echo "</pre>";*/
